...
# def ParseTree(doc: Document):
#     for sent in doc.sentences:
#         tree = sent.constituency
#         print(NLTREE.fromstring(str(tree)))
#         print()


# def FindSentence(child: Tree) -> List[Tree]:
#     '''
#     Find label 'S' from tree recursively and reconstruct to a sentence.
#     Notice: The input should not contains ROOT element.

#     Example
#     --------
#     >>> for sent in doc.sentences:
#             tree = sent.constituency  # tree contains root
#             FindSentence(tree.children[0]) #  return list of sentences
    
#     '''
#     if child.label!="S": 
#         return None
#     if "S" not in [x.label for x in child.children]:
#         return child
#     children = []
#     for c in child.children:
#         sents = FindSentence(c)
#         if isinstance(sents, list):
#             children.extend(sents)
#         elif isinstance(sents, Tree):
#            children.append(sents)
#     return children


# def UnpackVP(token: Tree) -> List[Tree]:
#     if token.label!="VP": return []
#     if token.is_leaf(): return token.leaf_labels()
#     verbs = []
#     for t in token.children:
#         vp = UnpackVP(t)
#         verbs.extend(vp)
#         if t.label in VERB:
#             verbs.extend(t.leaf_labels())
#     return verbs


# def UnpackNP(token: Tree):
#     if token.label != "NP": return []
#     if token.is_leaf(): return token.leaf_labels()
#     subs = []
#     objs = []
#     for t in token.children:
#         np_sub, np_obj = UnpackNP(t)
#         subs.extend(np_sub)
#         objs.extend(np_obj)
#         if t.label in 
        

# def FindSVO(sentence: Tree) -> Tuple[str,str,str]:
...
#     for sent in doc.sentences:
#         tree = sent.constituency
#         sentences = FindSentence(tree.children[0])
#         # parse tree into multiple sentences
#         for s in sentences:
#             print(NLTREE.fromstring(str(s)))
#             FindSVO(s)
#             print("==================")
...
def _find_svs(tokens):
    ...
    for v in verbs:
        ...
        if len(subs) > 0:
            for sub in subs:
                svs.append((sub.orth_, "not " + v.orth_ if verbNegated else v.orth_))
    return svs
            ...
            if isConjVerb:
                    ...
                    for obj in objs:
                        ...
                        if is_pas:  # reverse object / subject for passive
                            svos.append((to_str(expand(obj, tokens, visited)),
                                         "not " + v.lemma_ if verbNegated or objNegated else v.lemma_, to_str(expand(sub, tokens, visited))))
                            svos.append((to_str(expand(obj, tokens, visited)),
                                         "not " + v2.lemma_ if verbNegated or objNegated else v2.lemma_, to_str(expand(sub, tokens, visited))))
                        else:
                            svos.append((to_str(expand(sub, tokens, visited)),
                                         "not " + v.lower_ if verbNegated or objNegated else v.lower_, to_str(expand(obj, tokens, visited))))
                            svos.append((to_str(expand(sub, tokens, visited)),
                                         "not " + v2.lower_ if verbNegated or objNegated else v2.lower_, to_str(expand(obj, tokens, visited))))
            else:
                ...
                for sub in subs:
                    if len(objs) > 0:
                        ...
                        svos.append((to_str(expand(sub, tokens, visited)),
                                     "not " + v.lower_ if verbNegated else v.lower_,))
